...
var promise=$.ajax({
		...
		promise.then((res)=>{
			geo(res,latitud,longitud)
			var maxx=document.querySelector(".maxx")
			maxx.innerHTML=`<img src="/img/shared/spinner.gif"/>`
			//maxx.innerText="Actualizar ubicación"
			maxx.classList.remove("btn-primary")
			maxx.classList.remove("btn")
			//maxx.classList.add("btn-secondary")
...
var geo=(respuesta,latitud,longitud)=>{
	let abc=[]
	let idemp=[]
		for (let i = 0; i < respuesta.length; i++) {
					lat1 = latitud
					...
					var d = R * c;
					abc.push(d.toFixed(3));
					idemp.push(respuesta[i].emp)
				}
			condigeo(abc,idemp)
			}
var kms=[];
var condigeo=(km,idemp)=>{
	let templeate=document.querySelector("#templeateCercano")
		templeate.innerHTML="";
	contador=0
    for(let i=0;i<km.length;i++){
	if (km[i] <= 10 && contador<8) {
		kms.push(km[i])
	contador++
	var promise2=$.ajax({
			method: "GET",
		    url: base_url + "/rest/todosEmp/" + idemp[i]
	})
	promise2.then((res)=>{
	    console.log("km dento del then",kms[i])
	    console.log("km dento del then",kms)
			var maxx=document.querySelector(".maxx")
			maxx.innerHTML=``
			maxx.innerText="Actualizar ubicación"
			maxx.classList.add("btn-secondary")
			maxx.classList.add("btn")
			//maxx.classList.add("btn-secondary")
		...
		let tieneFoto=res[0]
    						...
    						<div class="slide-card slide-card2" style="background:url(${base_url}/foto/load/${res[4]}); background-size:cover" >
						...
						<p class="name name4" >Se encuentra a ${kms[i]}km</p>
         			  </div>
    			</div></span>
				</span>`}
				else{
...
let ponerFotoEmprendimiento2=document.querySelectorAll("#pregunta-consumidor .emp-header-min .camera")
...
entregado.forEach((element,key)=>{element.addEventListener("click",function(){
		...
		if(idFotoEmprendimiento[key]){
			...
			ponerFotoEmprendimiento2[0].innerHTML=""
		}
